...
"""
Fibonacci Sequence Modulo Period - Command Line Interactive 3D Visualization Tool

Usage:
    python3 fibonacci_cli_3d_visualizer.py
    
Features:
    - Interactive input of modulo
    - Select sequence ranges to visualize
    - Generate 3D visualization with cubes where height represents sequence period
...
class Fibonacci3DVisualizer:
    ...
    def print_banner(self):
        ...
        self.clear_screen()
        print("=" * 70)
        print("  Fibonacci Sequence Modulo Period - 3D Visualization Tool")
        print("=" * 70)
        print()
    ...
    def generate_visualization(self):
        """Generate 3D visualization with top cubes only where height represents sequence period"""
        if not self.selected_sequences:
        ...
        print("\nGenerating 3D visualization with top cubes only...")
        print(f"  - Modulo: {self.base}")
        ...
        for idx in self.selected_sequences:
            ...
            rgb_color = (r, g, b, 0.8)  # Add alpha for transparency
        ...
        ax.set_zlabel('Sequence Period Height')
        ax.set_title(f'3D Fibonacci Sequence Mod {self.base} Period Visualization\nTop cubes represent sequence period length', fontsize=14, fontweight='bold')
        ...
        ax.set_zlim(0, max_height * 1.2)  # Add some space above for better visualization
        
        # Add legend for sequence information
        legend_text = f"Modulo: {self.base}\n"
        legend_text += f"Total Sequences: {len(self.sequences)}\n"
        legend_text += f"Selected: {len(self.selected_sequences)}\n\n"
        
        for i, info in enumerate(sequence_info[:20]):
            legend_text += f"[{info['index']}] Length={info['length']}\n"
            legend_text += f"    {info['preview']}\n"
        ...
        # ========== Display and save ==========
        print("\n✓ 3D visualization generation completed!")
        print("  - Close chart window to return to menu")
        
        try:
            plt.show(block=True)
        except Exception as e:
            print(f"Warning: Problem displaying chart: {e}")
            print("  But chart file can still be saved.")
        
        # Ask if save
        save_choice = input("\nSave 3D chart as PNG files? (y/n): ").strip().lower()
        if save_choice == 'y':
            try:
                ...
                for angle in angles:
                    ...
                    filename = f"fibonacci_mod_{self.base}_3d_visualization_{angle['name']}.png"
                    fig.savefig(filename, dpi=150, bbox_inches='tight')
                    print(f"✓ 3D chart ({angle['name']} view) saved as: {filename}")
    ...
    def main_menu(self):
        ...
        while True:
            ...
            print("3. Generate 3D visualization charts")
...
def main():
    ...
    visualizer = Fibonacci3DVisualizer()
